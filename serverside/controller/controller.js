...
export async function addUser(req,res) {
    ...
    if(data)
        return res.status(404).send({msg:"username already existed"})
    ...
    // });
        await userSchema.create({username,password:hpassword}).then(()=>{
...
export async function loginUser(req,res){
    ...
    if(user==null)
        return res.status(404).send({msg:"username is not valid"})
...
export async function Home(req,res){
    ...
    try {
        ...
        res.status(200).send({username:user.username,msg:"Welcome to our World"})
    } catch (error) {
